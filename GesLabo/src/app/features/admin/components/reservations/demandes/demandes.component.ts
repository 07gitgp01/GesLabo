...
export class DemandesComponent {
  ...
  getListe() {
      ...
      updateStatut(reservationId: string, newStatut: string) {
        const reservation = this.liste.find(res => res.id === reservationId);
        if (reservation) {
          reservation.statut = newStatut; // Mettez à jour le statut
          // Vous pouvez également appeler une API pour enregistrer ce changement dans votre backend
          console.log(`Le statut de la réservation ${reservationId} a été mis à jour en ${newStatut}.`);
        }
      }
      // Méthode pour supprimer une réservation
      supprimerReservation(reservationId: string) {
        this.liste = this.liste.filter(res => res.id !== reservationId);
